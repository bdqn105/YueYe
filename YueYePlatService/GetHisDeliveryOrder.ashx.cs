...
{
    /// <summary>
    /// GetHisDeliveryOrder 的摘要说明
    /// </summary>
    public class GetHisDeliveryOrder : IHttpHandler, IRequiresSessionState
    {
                ...
                try
                {
                    ...
                    if (model != null)
                    {
                        ...
                        foreach (YueYePlat.Model.logiscompanyinfo m in logisList)
                        {
                            ...
                            //查询未完结的运输配送单号
                            List<YueYePlat.Model.vehicledelivery> deliveryList = new YueYePlat.BLL.vehicledelivery().GetModelList(String.Format("ifEnd=1 and BeginTime>DATE_ADD(NOW(),INTERVAL -{0} MONTH)", months));
                            string deliveries = "";
